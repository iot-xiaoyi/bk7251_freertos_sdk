...
/*
 * FIXME: wpa and hostapd share signal array.
 *
 * For example: SIGCSA issue.
 *   step1: sta and softap all starts, sta connects to AP who sits on channel 1, softap also on channel 1.
 *   step2: AP change to channel 2.
 *   step3: sta reconnect to AP
 *   step4: sta connected to AP, and SM_CONNECT_IND_
 *   step5: softap try to switch channel to 2, and register signal SIGCSA
 *   step6: sta disconnected from AP, because some reason, reason 7 for example
 *   step7: wpa_supplicant_deinit, and free-ed all signals
 *   step8: softap CSA completed, and try to handle SIGCSA, ** but it's free-ed **
 *   step9: new wpa_supplicant instance starts.
 *
 *   results to CSA failed, because EVENT_CH_SWITCH lost.
 *  We comment SIGCSA here to ensure CSA successful.
 */
void eloop_remove_sta_added_signals(void)
{
    ...
    //eloop_signals_remove_signal(SIGCSA);		// ???? added by hostapd, and free-ed by wpa_supplicant_deinit ??
}
