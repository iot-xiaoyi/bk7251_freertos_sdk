...
int __wpa_ctrl_request(wpa_ctrl_cmd_t cmd, void *data, int wait)
{
	wpah_msg_t msg;
	int result = -1;
	msg.cmd = cmd;
...
{
	return __wpa_ctrl_request(cmd, data, 1);
}

/* TODO: free */
int wpa_ctrl_rw_evt(int event, void *data)
{
	return __wpa_ctrl_request(WPA_CTRL_CMD_RW_EVT, (void *)event, 0);
}
	...
	struct wpa_supplicant *wpa_s, ScanResult_adv *results)
{
	...
	dl_list_for_each(bss, &wpa_s->bss_id, struct wpa_bss, list_id) {
		...
		ieee80211_freq_to_chan(bss->freq, &results->ApList[index].channel);
...
int wpa_supplicant_ctrl_iface_set_network(struct wpa_supplicant *wpa_s, wlan_sta_config_t *config)
{
	...
	case WLAN_STA_FIELD_SSID:
		...
		ssid->ssid = dup_binstr(config->u.ssid.ssid, config->u.ssid.ssid_len);
	...
	case WLAN_STA_FIELD_FAST_CONNECT:
		wpa_s->fast_connect = !!config->u.fast_connect;
		break;
	...
	case WLAN_STA_FIELD_PSK:
		if (config->u.psk[0] != '\0') {
			if (os_strcmp(ssid->passphrase, config->u.psk)) {
				ssid->psk_set = 0;
				ssid->mem_only_psk = 1;
			}
...
#define CHECK_HAPD()				\
	...
	case WPA_CTRL_CMD_SELECT_NETWORK:
		CHECK_WPA_S();
		wpa_supplicant_ctrl_iface_select_network(wpa_s, 0/*(int)msg->param*/);
		break;
