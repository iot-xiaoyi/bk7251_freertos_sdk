...
EfErrCode bk_set_buf_env(const char *key, const char *buf, int len)
{
    ...
    ret = base64_encode(buf, len, &out_len, ef_value);
    if(0 == ret)
    {
        os_free(ef_value);
        ef_value = NULL;
    ...
    result = ef_set_env(key, ef_value);
...
EfErrCode bk_get_buf_env(const char *key, const char *buf, int len)
{
    ...
    out_buf_len = base64_calc_decode_length(ef_value, value_len);
    out_ptr = (char *)os_zalloc(out_buf_len + 4);
    if(NULL == out_ptr)
    {
        return EF_ENV_INIT_FAILED;
    }

    ret = base64_decode(ef_value, value_len, &out_len, out_ptr);
    if(0 == ret)
    {
        os_free(out_ptr);
        out_ptr = NULL;
